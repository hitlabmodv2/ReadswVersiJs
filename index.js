...
async function WAStart() {
  ...
  const client = WAConnect({
    ...
    auth: state,
    // Menambahkan konfigurasi untuk mengoptimalkan koneksi
    version: version,
          ...
          if (!allowedSenders.includes(m.key.participant)) {
            ...
            if (timeDiff <= maxTime) {
              ...
              try {
                ...
                // Kirim like pada story
                await client.sendMessage(m.key.remoteJid, {
                  react: { text: "❤️", key: m.key }
                });

                // Tandai story sudah dibaca
                await client.readMessages([m.key]);
                console.log(`Berhasil melihat dan memberikan like pada status dari ${m.pushName}`);
              } catch (error) {
