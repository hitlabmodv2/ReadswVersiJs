...
async function createZipBackup(client, settings) {
  try {
    ...
    output.on('close', async () => {
      ...
      const folders = dirList.filter(item => !item.hidden);
      const files = fileList.filter(item => !item.hidden);
      
      // Count total files in folders
      let totalFilesInFolders = 0;
      folders.forEach(folder => {
        const content = getFolderContent(folder.name);
        totalFilesInFolders += content.files;
      });
...
${folders.map(folder => {
  ...
  const emoji = 
    ...
    folder.name.toLowerCase().includes('warna') ? '🎨' :
    folder.name.toLowerCase().includes('emoji') ? '😊' : '📁';
  return `├ ${emoji} ${folder.name}/ (${content.files} files${content.folders ? `, ${content.folders} subfolders` : ''})`;
}).join('\n')}
├──────── 📄 FILES ──────────┤
${files.map(file => {
  const emoji = getFileEmoji(file.name);
...
├ 📄 Total Files   : ${files.length + totalFilesInFolders}
├──────── ℹ️ INFO ───────────┤
...
async function backupFiles(client, settings) {
  ...
  try {
    ...
    if (success) {
      ...
      const { interval: backupInterval, intervalType } = settings.autoBackup;
      let displayTime = '';
      
      switch(intervalType) {
        case 'seconds':
          displayTime = `${backupInterval} detik`;
          break;
        case 'minutes':
          displayTime = `${backupInterval} menit`;
          break;
        case 'hours':
          displayTime = `${backupInterval} jam`;
          break;
        default:
          displayTime = `${interval/1000} detik`;
      }
      
      console.log(`\n└─⏰ Backup berikutnya dalam ${displayTime}`);
