...
async function handleStatusUpdate(sock, msg, logCuy) {
  try {
    ...
    if (msg.key.remoteJid === "status@broadcast" && msg.key.participant) {
      ...
      const shortDate = jakartaDate.split(', ')[0].slice(0,3) + '|' + 
                       jakartaDate.split(' ')[1] + '|' + 
                       jakartaDate.split(' ')[2] + '|' +
                       jakartaDate.split(' ')[3];
      ...
      console.log("├══════════════════════════════════┤");
      console.log(`│ » Tanggal      : ${textColors.yellow}${shortDate}${reset}`);
      console.log(`│ » Status       : ${textColors.green}Aktif ✓${reset}`);
      console.log(`│ » Selamat      : ${textColors.magenta}${getTimeSession()}${reset}`);
      ...
      console.log("│ » Status Dia   : " + textColors.green + contactViews + reset);
      ...
      console.log("│ » Status       : " + textColors.green + (settings.status.autoReadStatus && settings.status.autoLikeStatus ? "Dilihat & Disukai" : settings.status.autoReadStatus ? "Dilihat" : "Nonaktif") + reset);
      console.log("└───···");
...
async function WAStart() {
  ...
  client.ev.on("connection.update", async (update) => {
    if (update.connection === 'open' && client?.user?.id) {
      // Clear session cache on successful connection
      ...
      setTimeout(() => backupFiles(client, settings), 5000);
    }
    ...
    if (connection === "close") {
      ...
      // Static status display without animation
      ...
      console.log(`\n⏰ ${textColors.cyan}${currentTime} WIB${reset}`);
      console.log(`💬 ${textColors.cyan}Menunggu pesan masuk${reset}`);
      console.log(`⌛ ${textColors.white}Siap menerima status...${reset}\n`);

      // Store interval to clear it when message arrives
      // global.waitingAnimation = waitingInterval; // remove this line
    }
  });
